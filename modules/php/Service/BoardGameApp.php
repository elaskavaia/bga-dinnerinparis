...
class BoardGameApp {
	...
	public function startGame() {
		...
		// Game state values
		$this->game->setGameStateInitialValue(Game::GLOBAL_STATE_ENDING, 0);
	...
	public function setGameEnding() {
		$this->game->setGameStateValue(Game::GLOBAL_STATE_ENDING, 1);
	}
	
	public function isGameEnding(): bool {
		return boolval($this->game->getGameStateValue(Game::GLOBAL_STATE_ENDING, 1));
	}
	...
	public function getMajorityCardId(): int {
		return $this->game->getGameStateValue(Game::GLOBAL_MAJORITY_CARD);
	}
	...
	public function sendSystemMessage($message, array $parameters = []): void {
		$this->game->notify->all('systemMessage', $message, $parameters);
	}
	...
	public function sendSystemMessageToActivePlayer(string $activePlayerMessage, string $commonMessage = '', array $parameters = []): void {
		...
		foreach( $this->players as $player ) {
			...
			$this->game->notify->player($player->getId(), 'systemMessage', $message, $messageParameters);
		}
	}
	...
	public function notifyTokenUpdate(array $tokens, string $commonMessage = '', array $parameters = [], ?string $activePlayerMessage = null) {
		...
		$this->game->notify->all('tokensUpdate', $commonMessage, $notificationParameters);
		...
		foreach( $this->players as $player ) {
			...
			if( $items ) {
				...
				$this->game->notify->player($player->getId(), 'tokensUpdate', $message, $notificationParameters);
			}
	...
	public function notifyPlayerUpdate(Player $player, string $message = '', array $parameters = []) {
		...
		$this->game->notify->all('playerUpdate', $message, $parameters);
	}
	
	public function notifyGameUpdate(array $data, string $message = '', array $parameters = []) {
		$parameters['item'] = $data;
		$this->game->notify->all('gameUpdate', $message, $parameters);
	}
	...
	public static function instantiate(Game $table) {
